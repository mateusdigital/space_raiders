...
public:
	void SetGlyph(int x, int y, _char_t c)
	{
	...
	_char_t SampleGlyph(float x, float y)
	{
	...
	bool Save(std::string sFile)
	{
		...
		f.write((char*)m_Glyphs, sizeof(_char_t) * nWidth * nHeight);
	...
	bool Load(std::string sFile)
	{
		...
		f.read((char*)m_Glyphs, sizeof(_char_t) * nWidth * nHeight);
...
public:
	...
	virtual void Draw(int x, int y, _char_t c = PIXEL_SOLID, short col = 0x000F)
	{
	...
	void Fill(int x1, int y1, int x2, int y2, _char_t c = PIXEL_SOLID, short col = 0x000F)
	{
		Clip(x1, y1);
		Clip(x2, y2);
		for (int x = x1; x < x2; x++)
	...
	void DrawLine(int x1, int y1, int x2, int y2, _char_t c = PIXEL_SOLID, short col = 0x000F)
	{
	...
	void DrawCircle(int xc, int yc, int r, _char_t c = PIXEL_SOLID, short col = 0x000F)
	{
		int x = 0;
		int y = r;
		int p = 3 - 2 * r;
	...
	void FillCircle(int xc, int yc, int r, _char_t c = PIXEL_SOLID, short col = 0x000F)
	{
...
private:
	...
	// Handle Frame Update
				if (!OnUserUpdate(fElapsedTime))
					m_bAtomActive = false;
				// _char_t s[256];
				// snprintf(s, 256, " %s - FPS: %3.2f", m_sAppName.c_str(), 1.0f / fElapsedTime);
